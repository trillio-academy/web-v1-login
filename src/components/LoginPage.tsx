  ...
  const renderBrandingPanel = (options?: { showContinueButton?: boolean }) => (
    <>
      {cliente?.permitirTraducao ? (
        <div className="w-full flex justify-center mb-4" role="group" aria-label="Selecionar idioma">
          <div className="flex gap-2">
            <button type="button" className="w-8 h-8 rounded-full border-2 opacity-70 hover:opacity-100 transition-opacity flex items-center justify-center overflow-hidden bg-gray-100" style={{ borderColor: corFonteTelaLogin }} title="Português" aria-label="Selecionar Português" onClick={() => { document.cookie = 'locale=pt; path=/; max-age=31536000'; window.location.reload(); }}>
              <img src="https://flagcdn.com/w40/br.png" alt="" className="w-full h-full object-cover" width={32} height={32} />
            </button>
            <button type="button" className="w-8 h-8 rounded-full border-2 opacity-70 hover:opacity-100 transition-opacity flex items-center justify-center overflow-hidden bg-gray-100" style={{ borderColor: corFonteTelaLogin }} title="English" aria-label="Selecionar English" onClick={() => { document.cookie = 'locale=en; path=/; max-age=31536000'; window.location.reload(); }}>
              <img src="https://flagcdn.com/w40/us.png" alt="" className="w-full h-full object-cover" width={32} height={32} />
            </button>
            <button type="button" className="w-8 h-8 rounded-full border-2 opacity-70 hover:opacity-100 transition-opacity flex items-center justify-center overflow-hidden bg-gray-100" style={{ borderColor: corFonteTelaLogin }} title="Español" aria-label="Selecionar Español" onClick={() => { document.cookie = 'locale=es; path=/; max-age=31536000'; window.location.reload(); }}>
      ...
      <div className={`${options?.showContinueButton ? '' : 'flex-1'} flex flex-col items-center justify-center text-center px-4`}>
        ...
        <p className="mt-2 text-base md:text-lg font-light tracking-wide opacity-80" style={{ color: corFonteTelaLogin }}>
  ...
  return (
    <div className="min-h-screen flex items-start md:items-center justify-center py-4 md:py-8" style={gradientStyle}>
      <div className="w-full max-w-6xl mx-auto px-4">
        ...
        <div className={`${showMobileLogin ? 'flex' : 'hidden'} md:flex flex-col md:flex-row shadow-2xl rounded-lg overflow-hidden md:max-h-[608px]`}>
          <div
            className="hidden md:flex w-full md:w-2/5 flex-col justify-between items-center px-4 py-6 sm:px-8 md:px-12 md:py-12 md:min-h-[608px] rounded-b-lg md:rounded-none md:rounded-l-lg"
            style={cardRightStyle}
          >
            {renderBrandingPanel()}
          </div>
                    ...
                    >
                      {showPassword ? (
                        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                          <path d="M17.94 17.94A10.94 10.94 0 0 1 12 20C7 20 2.73 16.11 1 12c.74-1.76 1.91-3.31 3.37-4.53" />
                          <path d="M10.58 10.58a2 2 0 1 0 2.83 2.83" />
                          <path d="M1 1l22 22" />
                          <path d="M9.88 4.24A10.94 10.94 0 0 1 12 4c5 0 9.27 3.89 11 8a10.97 10.97 0 0 1-4.16 5.07" />
                        </svg>
                      ) : (
                        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
